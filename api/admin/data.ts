...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Public GET endpoint for newsletter unsubscribe (no auth required)
  if (req.method === 'GET' && type === 'newsletter-unsubscribe') {
    const token = req.query.token as string;
    if (!token) {
      return res.status(400).send(unsubPage('Ungültiger Link', 'Der Abmelde-Link ist ungültig.'));
    }
    const subscribers = await readNewsletterSubscribers();
    const subscriber = subscribers.find(s => s.unsubscribeToken === token);
    if (!subscriber) {
      return res.status(404).send(unsubPage('Nicht gefunden', 'Der Abmelde-Link ist ungültig oder wurde bereits verwendet.'));
    }
    if (subscriber.status === 'unsubscribed') {
      return res.status(200).send(unsubPage('Bereits abgemeldet', 'Sie wurden bereits vom Newsletter abgemeldet.'));
    }
    subscriber.status = 'unsubscribed';
    await writeNewsletterSubscribers(subscribers);
    return res.status(200).send(unsubPage('Erfolgreich abgemeldet', 'Sie wurden vom Newsletter der Alten Post Brensbach abgemeldet und erhalten keine weiteren Info-Post-Mails.'));
  }
